...
Before using your own data, validate the setup:
...
# 3) Sanity-check Streamlit upload flow
streamlit run test_upload_minimal.py
```

**What to expect:**  
- The app loads, you can upload `sample_customer_data.csv`, and you should see columns listed without errors.

If you see issues:
- Verify Python version (3.10–3.12 recommended).  
- Ensure your `.env` contains needed keys (e.g., `OPENAI_API_KEY`) if you run the full `streamlit_app.py`.  
...
(Add 2–3 images or a GIF that show:)
1. Home screen of the Streamlit app.
2. Data upload + preview.
3. “Generate EDA Report” action and the resulting **.docx** download button.

---

## 🐞 Troubleshooting (Extended)

- **API Key Error**: Ensure `.env` contains `OPENAI_API_KEY` and has credits.  
- **File Not Found**: Place datasets in the repo root or provide an absolute path.  
- **Import Errors**: Run `pip install -r requirements.txt` again.  
- **Memory Issues**: Use smaller CSVs to test before scaling.  
- **Streamlit Port Conflict**: Edit `run_streamlit.py` to change `--server.port 8501` to a free port.
